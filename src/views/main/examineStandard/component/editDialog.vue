				...
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="检验标准编号" prop="examineStandardCode">
							<el-input v-model="ruleForm.examineStandardCode" placeholder="请输入检验标准编号" maxlength="100" show-word-limit clearable />
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="检验标准名称" prop="examineStandardName">
							<el-input v-model="ruleForm.examineStandardName" placeholder="请输入检验标准名称" maxlength="100" show-word-limit clearable />
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="检验项目" prop="examineProjectId">
							<el-select clearable filterable v-model="ruleForm.examineProjectId" placeholder="请选择检验项目">
								<el-option v-for="(item,index) in examineProjectExamineProjectIdDropdownList" :key="index" :value="item.value" :label="item.label" />
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addExamineStandard(values);
				} else {
					await updateExamineStandard(values);
				}
				closeDialog();
			} else {
				ElMessage({
