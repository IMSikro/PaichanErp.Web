				...
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="工艺标准编号" prop="processStandardCode">
							<el-input v-model="ruleForm.processStandardCode" placeholder="请输入工艺标准编号" maxlength="100"
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="工艺标准名称" prop="processStandardName">
							<el-input v-model="ruleForm.processStandardName" placeholder="请输入工艺标准名称" maxlength="100"
								show-word-limit clearable />
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="工艺项目" prop="processProjectId">
							<el-select clearable filterable v-model="ruleForm.processProjectId" placeholder="请选择工艺项目">
								<el-option v-for="(item,index) in processProjectProcessProjectIdDropdownList"
									:key="index" :value="item.value" :disabled="disabledOptions[index]" :label="item.label" />
...
<script lang="ts" setup>
	...
	const rules = ref<FormRules>({
		processProjectId: [{ required: true, message: '请选择工艺项目！', trigger: 'change', },],
	...
	const openDialog = (row : any) => {
		...
		// console.log(props.processProjectNameList)
		// console.log(processProjectProcessProjectIdDropdownList.value)
		isShowDialog.value = true;
	};
	
	const disabledOptions = computed(() => {
		let list=props.processProjectNameList
		console.log(list)
	  return processProjectProcessProjectIdDropdownList.value.map(option => list.includes(option.label));
	});
	...
	const submit = async () => {
		...
		let processProjectName=processProjectProcessProjectIdDropdownList.value.filter(item => item.value === ruleForm.value.processProjectId)
		console.log(props.id.id)
		const produceFormulaIds={
			produceFormulaId:props.id.id,
			deviceTypeName:deviceTypeName[0].label,
			processProjectName:processProjectName[0].label
		}
		ruleFormRef.value.validate(async (isValid : boolean, fields ?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addProcessStandard(valueItem);
				} else {
					await updateProcessStandard(valueItem);
				}
				closeDialog();
			} else {
				ElMessage({
