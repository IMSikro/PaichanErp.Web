				...
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="产品编号" prop="produceId">
							<el-select clearable filterable v-model="ruleForm.produceId" placeholder="请选择产品编号">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="工艺名称" prop="processName">
							<el-input v-model="ruleForm.processName" placeholder="请输入工艺名称" maxlength="50" show-word-limit clearable />
...
<script lang="ts" setup>
	...
	import { addProduceProcess, updateProduceProcess } from "/@/api/main/produceProcess";
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addProduceProcess(values);
				} else {
					await updateProduceProcess(values);
				}
				closeDialog();
			} else {
				ElMessage({
