				...
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="计量单位" prop="unitName">
							<el-input v-model="ruleForm.unitName" placeholder="请输入计量单位" maxlength="100" show-word-limit
								clearable />
...
import { addSystemUnit, updateSystemUnit } from "/@/api/main/systemUnit";
	...
	ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			...
			if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
				await addSystemUnit(values);
			} else {
				await updateSystemUnit(values);
			}
			closeDialog();
		} else {
			ElMessage({
